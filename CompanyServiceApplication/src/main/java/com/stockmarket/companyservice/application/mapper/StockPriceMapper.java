package com.stockmarket.companyservice.application.mapper;
...
public class StockPriceMapper 
{
	...
	public List<StockPrice> toStockPrices(List<StockPriceDto> stockPrices)
	{
		ModelMapper mapper = new ModelMapper();
		mapper.getConfiguration().setMatchingStrategy(MatchingStrategies.STRICT);
		List<StockPrice> stockPriceDtos = Arrays.asList(mapper.map(stockPrices, StockPrice[].class));
		return stockPriceDtos;
	}
}
